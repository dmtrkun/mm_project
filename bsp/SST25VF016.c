...
BYTE SST25ReadByte(DWORD address)
{
    BYTE    temp;
  	...
  	CS_MUX(CS5_EEPROM,1);// Enable SPI Communication to  25VF016
  	
		while(SPI1STATbits.SPITBF);
  	WriteSPI1(SST25_CMD_READ);
  	while(!SPI1STATbits.SPIRBF);
  	ReadSPI1();
		
		while(SPI1STATbits.SPITBF);
  	WriteSPI1(((DWORD_VAL) address).v[2]);
  	while(!SPI1STATbits.SPIRBF);
  	ReadSPI1();
    
		while(SPI1STATbits.SPITBF);
  	WriteSPI1(((DWORD_VAL) address).v[1]);
  	while(!SPI1STATbits.SPIRBF);
  	ReadSPI1();

		while(SPI1STATbits.SPITBF);
  	WriteSPI1(((DWORD_VAL) address).v[0]);
  	while(!SPI1STATbits.SPIRBF);
  	ReadSPI1();
  	...
  	while(!SPI1STATbits.SPIRBF);
  	temp = ReadSPI1();
		CS_MUX(CS5_EEPROM,0);// Disable SPI Communication to  25VF016
...
//    while(SST25IsWriteBusy());
	stat_dbg = SST25ReadStat();
	while(SST25ReadStat()&0x01);
...
//    SST25CSLow();
  	...
  	while(SPI1STATbits.SPIRBF)
		...
		while(SPI1STATbits.SPITBF);
  	WriteSPI1(SST25_CMD_WRSR);
  	while(!SPI1STATbits.SPIRBF);
  	ReadSPI1();

		while(SPI1STATbits.SPITBF);
  	WriteSPI1(0); // clear all bits
  	while(!SPI1STATbits.SPIRBF);
  	ReadSPI1();
...
BYTE SST25WriteArray(DWORD address, BYTE *pData, WORD nCount)
{
    ...
    for(counter = 0; counter < nCount; counter++)
    {
        dbg_data = SST25ReadByte(address);
		if(*pData != dbg_data)
