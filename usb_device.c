...
////Depricated in v2.2 - will be removed in a future revision
    ...
    //Device descriptor
    extern ROM USB_DEVICE_DESCRIPTOR device_dsc0;
    ...
    if(SetupPkt.bmRequestType == 0x80)
    {
        ...
        switch(SetupPkt.bDescriptorType)
        {
            case USB_DESCRIPTOR_DEVICE:
                #if !defined(USB_USER_DEVICE_DESCRIPTOR)
                    if(cdc_ena)
	                    inPipes[0].pSrc.bRom = (ROM BYTE*)&device_dsc1;
										else
	                    inPipes[0].pSrc.bRom = (ROM BYTE*)&device_dsc0;
                #else
...
//                    inPipes[0].pSrc.bRom = *(USB_USER_CONFIG_DESCRIPTOR+SetupPkt.bDscIndex);
                    inPipes[0].pSrc.bRom = *(USB_USER_CONFIG_DESCRIPTOR+(cdc_ena)+SetupPkt.bDscIndex);
                #endif
...
//                    inPipes[0].pSrc.bRom = *(USB_SD_Ptr + SetupPkt.bDscIndex);
                    inPipes[0].pSrc.bRom = USB_SD_Ptr[(cdc_ena*4) + SetupPkt.bDscIndex];
                    // Set data count
