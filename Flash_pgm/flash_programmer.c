...
TEST_STRUCT tst_str[256];
uint16_t tst_num;
uint16_t tst_verif_flg;
/*****************************************************************************
 * bool BinaryHandlePacket(void)
 *****************************************************************************/
bool BinaryHandlePacket(void)
...
//        	pDataChipEraseFunc();
 ...
 //           if(memPayload->addr ==0x000033E0)
 //               DelayMs(1000);


//        WORD    pD;
//        DWORD   addr = memPayload->addr>>1;
//        for(addr = 0; addr < (0x80000>>1); addr++)
//        {
//            pD = lRead16(addr);
//            if(pD!=0xffff)
...
//        for(; addr < ((memPayload->addr + hdr->length)>>1); addr++)
//        {
//            pD = lRead16(addr);
//            if(pD!=0xffff)
//            {
...
//        }
            ...
            tst_str[tst_num].addr = memPayload->addr;
            tst_str[tst_num].length = hdr->length;
            tst_str[tst_num].num = tst_num;
            tst_num++;
