...
#if   (((GFX_EPMP_CS1_MEMORY_SIZE/2) > 0x10000ul)  && ((GFX_EPMP_CS1_MEMORY_SIZE/2) <= 0x20000ul))
    PMCON3 |= 0x0001; // PMA16 address line is enabled
#elif (((GFX_EPMP_CS1_MEMORY_SIZE/2) > 0x20000ul)  && ((GFX_EPMP_CS1_MEMORY_SIZE/2) <= 0x40000ul))
    PMCON3 |= 0x0003; // PMA[17:16] address lines are enabled
#elif (((GFX_EPMP_CS1_MEMORY_SIZE/2) > 0x40000ul)  && ((GFX_EPMP_CS1_MEMORY_SIZE/2) <= 0x80000ul))
    PMCON3 |= 0x0007; // PMA[18:16] address lines are enabled
#elif (((GFX_EPMP_CS1_MEMORY_SIZE/2) > 0x80000ul)  && ((GFX_EPMP_CS1_MEMORY_SIZE/2) <= 0x100000ul))
    PMCON3 |= 0x000F; // PMA[19:16] address lines are enabled
#elif (((GFX_EPMP_CS1_MEMORY_SIZE/2) > 0x100000ul) && ((GFX_EPMP_CS1_MEMORY_SIZE/2) <= 0x200000ul))
    PMCON3 |= 0x001F; // PMA[20:16] address lines are enabled
#elif (((GFX_EPMP_CS1_MEMORY_SIZE/2) > 0x200000ul) && ((GFX_EPMP_CS1_MEMORY_SIZE/2) <= 0x400000ul))
    PMCON3 |= 0x003F; // PMA[21:16] address lines are enabled
#elif (((GFX_EPMP_CS1_MEMORY_SIZE/2) > 0x400000ul) && ((GFX_EPMP_CS1_MEMORY_SIZE/2) <= 0x800000ul))
...
//    PMCS2BS = (GFX_EPMP_CS2_BASE_ADDRESS >> 8); // CS2 start address
    ...
    //      - assumes that the PMA[x] signal/pin is mapped to an actual device
#if   (((GFX_EPMP_CS2_MEMORY_SIZE/2) > 0x10000ul)  && ((GFX_EPMP_CS2_MEMORY_SIZE/2) <= 0x20000ul))
    PMCON3 |= 0x0001; // PMA16 address line is enabled
#elif (((GFX_EPMP_CS2_MEMORY_SIZE/2) > 0x20000ul)  && ((GFX_EPMP_CS2_MEMORY_SIZE/2) <= 0x40000ul))
    PMCON3 |= 0x0003; // PMA[17:16] address lines are enabled
#elif (((GFX_EPMP_CS2_MEMORY_SIZE/2) > 0x40000ul)  && ((GFX_EPMP_CS2_MEMORY_SIZE/2) <= 0x80000ul))
    PMCON3 |= 0x0007; // PMA[18:16] address lines are enabled
#elif (((GFX_EPMP_CS2_MEMORY_SIZE/2) > 0x80000ul)  && ((GFX_EPMP_CS2_MEMORY_SIZE/2) <= 0x100000ul))
    PMCON3 |= 0x000F; // PMA[19:16] address lines are enabled
#elif (((GFX_EPMP_CS2_MEMORY_SIZE/2) > 0x100000ul) && ((GFX_EPMP_CS2_MEMORY_SIZE/2) <= 0x200000ul))
    PMCON3 |= 0x001F; // PMA[20:16] address lines are enabled
#elif (((GFX_EPMP_CS2_MEMORY_SIZE/2) > 0x200000ul) && ((GFX_EPMP_CS2_MEMORY_SIZE/2) <= 0x400000ul))
    PMCON3 |= 0x003F; // PMA[21:16] address lines are enabled
#elif (((GFX_EPMP_CS2_MEMORY_SIZE/2) > 0x400000ul) && ((GFX_EPMP_CS2_MEMORY_SIZE/2) <= 0x800000ul))
    PMCON3 |= 0x007F; // PMA[22:16] address lines are enabled
#endif           
